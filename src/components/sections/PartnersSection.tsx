...
export function PartnersSection() {
  ...
  return (
    <section className="px-4 sm:px-8 lg:px-12 py-10 sm:py-14 max-w-[1380px] mx-auto w-full">
        ...
        <div className="w-full grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-6 sm:gap-8 items-center justify-center">
          {partners.map((partner) => {
            ...
            return (
              <div
                key={partner.name}
                className="group flex flex-col items-center justify-center gap-2 p-3.5 sm:p-4 rounded-2xl transition-all duration-300 hover:bg-black/[0.02]"
              >
                <div className="flex items-center gap-2.5 grayscale opacity-60 transition-all duration-300 group-hover:grayscale-0 group-hover:opacity-100 group-hover:scale-105">
                  ...
                  <span className="font-display text-sm sm:text-base font-bold tracking-tight text-[#0A2B1D]">
                    {partner.name}
                  </span>
                </div>
              </div>
