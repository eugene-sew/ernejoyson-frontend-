...
export function WhyAgroneSection() {
  ...
  return (
    <section id="services" className="px-3 sm:px-8 lg:px-12 py-12 sm:py-24 max-w-[1380px] mx-auto w-full space-y-8 sm:space-y-12">
      ...
      <div className="text-center max-w-3xl mx-auto space-y-3 px-2 sm:px-0">
        ...
        {/* Heading */}
        <h2 className="font-display text-3xl sm:text-5xl lg:text-6xl font-extrabold tracking-tight text-[#0A2B1D] leading-[1.1]">
          Why Farmers Choose ERNEJOYSON.
        </h2>

        {/* Supporting copy */}
        <p className="text-sm sm:text-lg text-[#0A2B1D]/80 leading-relaxed font-semibold max-w-xl mx-auto">
      ...
      <div className="rounded-none sm:rounded-[52px] bg-transparent sm:bg-[#FAF5ED] sm:border sm:border-[#EAE4D6] p-0 sm:p-10 lg:p-12 space-y-6 sm:space-y-10 shadow-none sm:shadow-lg">
        {/* Top Feature Rows */}
          ...
          {accordions.map((item) => {
            ...
            if (isOpen) {
              return (
                <div
                  ...
                  className="rounded-2xl sm:rounded-3xl bg-white p-4 sm:p-7 shadow-sm border border-black/5 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 sm:gap-5 transition-all duration-200 cursor-pointer ring-1 ring-[#0A2B1D]/10"
                >
                  <div className="flex-1 space-y-1.5 sm:space-y-2">
                    <div className="flex items-center gap-2">
                      <CheckCircle2 className="h-4.5 w-4.5 sm:h-5 sm:w-5 text-[#8BD333] shrink-0" />
                      <h3 className="font-display text-base sm:text-xl font-bold text-[#0A2B1D]">
                        {item.title}
                      </h3>
                    </div>
                    <p className="text-xs sm:text-base text-[#0A2B1D]/80 max-w-xl leading-relaxed font-medium pl-6 sm:pl-7">
                      {item.description}
                    </p>
                  </div>

                  {/* Right Crop Field Thumbnail */}
                  <div className="h-20 w-32 sm:h-28 sm:w-48 shrink-0 rounded-xl sm:rounded-2xl overflow-hidden bg-[#F4F1E8] shadow-sm">
                    <img
                      src={item.image}
                      alt={item.title}
            ...
            return (
              <div
                key={item.id}
                onClick={() => handleAccordionClick(item.id)}
                className="rounded-xl sm:rounded-2xl bg-white px-4 sm:px-7 py-4 sm:py-6 shadow-sm border border-black/5 flex items-center justify-between transition-all duration-200 hover:bg-white/95 cursor-pointer"
              >
                <h3 className="font-display text-sm sm:text-lg font-bold text-[#0A2B1D]">
                  {item.title}
                </h3>
                <span className="font-display text-xs sm:text-sm font-bold text-[#0A2B1D]/60">
                  {item.tag}
                </span>
              </div>
            )
        ...
        <div className="space-y-3 sm:space-y-4">
          ...
          <div className="flex items-center justify-between px-1 sm:px-2">
            <div className="flex items-center gap-2">
              <span className="inline-flex items-center gap-1.5 rounded-full bg-[#0A2B1D] px-3.5 sm:px-4 py-1.5 text-[11px] sm:text-xs font-bold text-[#8BD333] shadow-sm">
                <Video className="h-3.5 w-3.5 sm:h-4 sm:w-4" />
                Featured Media
              </span>
              <span className="text-xs sm:text-sm font-semibold text-[#0A2B1D]/70 hidden sm:inline">
            ...
            <div className="flex items-center gap-2">
              <button
                ...
                className="flex h-8 w-8 sm:h-9 sm:w-9 items-center justify-center rounded-full bg-white border border-[#0A2B1D]/20 text-[#0A2B1D] shadow-sm hover:bg-[#0A2B1D] hover:text-white transition-all cursor-pointer"
              >
              ...
              <div className="relative p-5 sm:p-7 sm:pb-8 flex flex-col justify-between min-h-[190px] sm:min-h-[220px] filter drop-shadow-2xl">
                ...
                <div className="relative z-10 mt-3 sm:mt-4 flex items-center gap-2 sm:gap-2.5 pt-1">
                  ...
                  <a
                    ...
                    className="flex h-8 w-8 sm:h-9 sm:w-9 items-center justify-center rounded-full bg-[#0A2B1D] text-[#8BD333] transition-all hover:bg-[#154631] active:scale-95"
                  >
